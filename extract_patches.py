...
if __name__ == "__main__":
    ...
    save_root = "/content/hovernet/dataset/training_data/%s/" % dataset_name

    # a dictionary to specify where the dataset path should be
    dataset_info = {
        "train": {
            "img": (".tiff", "/content/hovernet/data/images/"),
            "ann": (".mat", "/content/hovernet/data/labels/"),
        },
        "valid": {
            "img": (".tiff", "/content/hovernet/data/images/"),
            "ann": (".mat", "/content/hovernet/data/labels/"),
    ...
    for split_name, split_desc in dataset_info.items():
        ...
        for file_idx, file_path in enumerate(file_list):
            ...
            # Assuming img and ann are loaded
            print(f"Original img shape: {img.shape}")
            print(f"Original ann shape: {ann.shape}")

            # Resize img to match ann's dimensions
            img = cv2.resize(img, (ann.shape[1], ann.shape[0]))

            print(f"Adjusted img shape: {img.shape}")
            print(f"Adjusted ann shape: {ann.shape}")
